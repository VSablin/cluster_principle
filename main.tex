...
Naively, one could build the ansatz \eqref{eq:S0_2} without the step functions. This is true for scatterers with a unique stable state \cite{Xu2013}. Actually, if we take $M=1$ in Eq. \eqref{eq:S0_2}, we recover this structure
\begin{equation}\label{eq:S0_2_1}
S^0_{y_1y_2x_1x_2} = S_{y_1x_1}S_{y_2x_2} + S_{y_1x_2}S_{y_2x_1},
\end{equation}
where the step functions disappear. However, the step functions are necessary to fulfil causality if $M>1$, as we show in the following. We consider a two-photon input state
...
\end{align}
where $\xi_n^\dagger$ generates a photon whose state is $\ket{\phi_n}$, with $n=a,b,a',b'$, and $\ket{\text{vac}}$ is the photonic state without particles. As $\xi_n^\dagger$ is a bosonic operator, both states are already symmetrized. According to the input-output theory, the probability amplitude between both states, Eq. \eqref{eq:A}, reads \cite{Fan2010}
...
\begin{align}
\xi_{n,\text{in}}^\dagger & \equiv e^{iHt_-} e^{-iH_0t_-}\xi_n^\dagger e^{iH_0 t_-} e^{-iHt_0},\\
\xi_{n',\text{out}}^\dagger & \equiv e^{iHt_+} e^{-iH_0t_+}\xi_{n'}^\dagger e^{iH_0 t_+} e^{-iHt_+},
\end{align}
with $t_-\to-\infty$ and $t_+\to\infty$ {\color{blue}($H$ and $H_0$ should be defined in Section II)}. As we want to find Eq. \eqref{eq:A}, we need the following relations to fulfil
\begin{align}
[\xi_{a',\text{out}},\xi_{b,\text{in}}^\dagger]&=0, \label{eq:comm_1} \\
[\xi_{b',\text{out}},\xi_{a,\text{in}}^\dagger]&\neq 0. \label{eq:comm_2}
\end{align}
If they are true, Eq. \eqref{eq:A_inout} can be rewritten as
\begin{align}
...
\end{align}
We need the equation relating $a_\text{in/out}(t)$ in order to evaluate the previous commutators. For the sake of simplicity, we particularize this computation for a $\lambda$ atom: $M=2$ and an only unstable state, with decay rates $\gamma_i$. The equation, derived in the Appendix, is
\begin{equation}
...
\end{align}
The first term of the right hand side vanishes, since $a_\text{in}^\dagger(t)$ can be written as an integral of Heisenberg operators evaluated at time $t_-$, $a_k^\dagger(t_-)$ \cite{Fan2010,Xu2015} and $[\sigma_{\lambda e},a_k^\dagger]=0$, since they belong to different Hilbert spaces. For the other term, as argued in \cite{Xu2015}, the first argument of the commutator just depends on $\{a_\text{in}(t')\}$, for $t_-<t'<t_1$. Thus, if $t_1<t_2$, this commutator is zero, and otherwise it is not. In consequence, $[\sigma_{\lambda e}(t_1),a_\text{in}^\dagger(t_2)]=0$ if $t_1<t_2$ and $[\sigma_{\lambda e}(t_1),a_\text{in}^\dagger(t_2)]\neq 0$ if $t_1>t_2$. Introducing these conditions in \eqref{eq:comm_in_out}
\begin{align}
...
We conclude the conditions \eqref{eq:comm_causality_1} and \eqref{eq:comm_causality_2} are necessary to fulfil causality.%Otherwise, the scattering process would not be able to distinguish the order of the incident photons. This condition also applies for systems with a unique stable state. However, in such a case, the probability amplitude \eqref{eq:A} is
%\begin{equation}
...
{\color{blue}
\begin{enumerate}
\item $\lambda$ atom with $N=2$ in momentum space \cite{Xu2016}. Fluorescence from the new part of $S^0$. How the fluorescence decays. MAYBE NOT
\item Ultrastrong MPS. Fig. with resonance fluorescence. We might include a figure with the decay of the fluorescence as a function of $L$. \cite{Sanchez-Burillo2014,Sanchez-Burillo2015}. MAYBE NOT
\end{enumerate}
}
...
We need to know the Heisenberg equations in order to derive the input-output ones. The Heisenberg equation for $a_\omega(t)$ is
...
\end{align}
The operators $\sigma_{\lambda e}^z=\ket{e}\bra{e}-\ket{g_\lambda}\bra{g_\lambda}$ and $\sigma_{\lambda\overline{\lambda}}=\ket{g_\lambda}\bra{g_{\overline{\lambda}}}$ have already been defined in the main text. As usual, $\overline{\lambda}\neq \lambda$.
...
I define
\begin{align}
\Phi(t) & \equiv \frac{1}{\sqrt{2}}\int d\omega\; a_\omega (t),\\
...
\end{align}
The times $t_\pm$ are $t_\pm\to \pm\infty$. Integrating Eq. \eqref{eq:aw_t} with respect to $\omega$ and taking into account that the decay rates are related to the coupling constants as $\gamma_\lambda=\pi g_\lambda^2$
\begin{equation}\label{eq:phi_in}
\Phi(t)=a_\text{in}(t)-\frac{i}{\sqrt{2}}\sum_{\lambda=1}^2\sqrt{\gamma_\lambda} \sigma_{\lambda e}(t).
\end{equation}
Repeating the same procedure, integrating Eq. \eqref{eq:aw_t} from $t$ to $t_+$
\begin{equation}\label{eq:phi_out}
\Phi(t)=a_\text{out}(t)+\frac{i}{\sqrt{2}}\sum_{\lambda=1}^2\sqrt{\gamma_\lambda} \sigma_{\lambda e}(t).
\end{equation}
Form Eqs. \eqref{eq:phi_in} and \eqref{eq:phi_out}
\begin{equation}
a_\text{out}(t)=a_\text{in}(t)-i\sum_{\lambda=1}^2 \sqrt{2\gamma_\lambda} \sigma_{\lambda e}(t),
\end{equation}
which is equal to Eq. \eqref{eq:a_inout}.
