...
A_{ab\to a'b'}^{\nu\to\mu} \equiv \langle \Psi_\text{out}|S^0|\Psi_\text{in}\rangle =  \sum_\lambda A_{a\to a'}^{\nu\to\lambda} A_{b\to b'}^{\lambda\to\mu},
\end{equation}
...
&(S_{p_1p_2k_1k_2}^0)_{\mu\nu}=\frac{1}{(2\pi)^2} \int dy_1 dy_2\; e^{-i(p_1y_1+p_2y_2)} \sum_{n=1}^2 e^{i(k_ny_1 + k_{\overline{n}} y_2)} \nonumber\\
& \sum_{m=1}^2 e^{i[(E_\lambda-E_\mu)y_m+(E_\nu-E_\lambda)y_{\overline{n}}]}t_{\mu\lambda}(k_n)t_{\lambda\nu}(k_{\overline{n}}) \theta(y_{\overline{m}}-y_m),
\end{align}
...
\end{align}
Notice the integral in $y_2$ is just a Dirac delta. Thus
\begin{align}
...
\begin{align}
(S_{p_1p_2k_1k_2}^0)_{\mu\nu}=  &\frac{i}{2\pi}\sum_{n,m=1}^2 \sum_\lambda  \frac{t_{\mu\lambda}(k_n) t_{\lambda\nu}(k_{\overline{n}})}{p_m+E_\mu -k_n -E_\lambda + i0^+}\nonumber\\
&\delta(p_1+p_2+E_\mu - k_1-k_2-E_\nu),
\end{align}
...
|\Psi_\text{out}^1\rangle = S|\Psi_\text{in}\rangle = \sum_\mu \int dy dx\;(S_{yx})_{\mu\nu}f_a(x)|y\rangle|g_\mu\rangle.
\end{equation}
Defining
...
\begin{equation}
|\Psi_\text{out}^1\rangle = \sum_\mu |\psi_\text{out}^1\rangle_{a,\mu\nu}|g_\mu\rangle.
\end{equation}
...
\mathbb{I}=\frac{1}{2}\sum_\mu \int dx_1dx_2\;|x_1x_2;g_\nu\rangle \langle x_1x_2;g_\nu|,
\end{equation}
...
&\sum_{\mu\lambda}\sum_{n,m}(S_{y_nx_m})_{\mu\lambda} (S_{y_{\overline{n}}x_{\overline{m}}})_{\lambda\nu}\theta(y_{\overline{n}}-y_n)\nonumber\\
...
C=\sum_n(&\phi_{i,\mu\lambda}(y_n)\phi_{j,\lambda\nu}(y_{\overline{n}}) \nonumber\\
...
C=\sum_n\phi_{b,\mu\lambda}(y_n)\phi_{a,\lambda\nu}(y_{\overline{n}})).
\end{align}
One can see easily that the same expression is true if $i=b$ and $j=a$. Then, the output state, Eq. \eqref{eq:out2_2} is
\begin{align}
|\Psi_\text{out}^2\rangle &=\frac{1}{2\sqrt{2}}\int dy_1dy_2\;\sum_{\mu\lambda}(\phi_{b,\mu\lambda}(y_1)\phi_{a,\lambda\nu}(y_2)\nonumber\\
...
|\Psi_\text{out}^2\rangle =&\frac{1}{\sqrt{2}}\sum_{\mu\lambda}(|\psi_\text{out}^1\rangle_{b,\mu\nu}|\psi_\text{out}^1\rangle_{a,\nu\lambda}  \nonumber\\
