...
\section{Free dynamics of asymptotic wavepackets / asymptotic states / well defined scattering matrix}
...
\begin{equation}
|\Psi_\text{in}\rangle = \left(\prod_{n=1}^N \psi_{k_n}(x_n)^\dagger \right)|\Omega\rangle,
\end{equation}
with $|x_n-x_m|\to\infty$ for all $n\neq m$. The scattering amplitude of going to a general output state will be
\begin{equation}
A_{\text{in}\to\text{out}}=\bra{\Psi_\text{out}}S\ket{\Psi_\text{in}}.
\end{equation}
Our output state will be also composed by separated wavepackets
\begin{equation}
|\Psi_\text{out}\rangle = \left(\prod_{n=1}^N \psi'_{p_n}(y_n)^\dagger \right)|\Omega\rangle,
\end{equation}
