...
\label{H-full}
\end{equation}
We have chosen to write a discretised version (with steps $\delta_k$) rather than a continuum one for the photonic medium in momentum space   In the Hamiltonian, $\omega_k$ are the photon frequencies and $[a_k, a_k^\prime]= \delta_{k, k^\prime}$ bosonic operators.  We assume a single band photons, though multiband systems should give the same conclusions here.  Thus, $\omega_k \in [\omega_-, \omega_+]$ wich will be referred as the band limits.  In a field theory nomenclature they are the Infrarred and Ultraviolet cutoffs.  The system-bosonic medium is via the operator $S$ with coupling constants $g_k$.    The system Hamiltonian  can be  any.  
Through the paper we will fix to one quantum system.  The extension to several scatterers  is trivial and do not change our conclusions.  When transforming to real space,
\begin{equation}
a_x = \frac{1}{\sqrt{L} }\sum_k  \, e^{i k x} a_k  
\end{equation}
with $L$ the number of latice sites, $\delta_k= \pi /L$, the punctual scatterer is assumed to occupy a finite number of sites, \emph{i.e.} $g_x = 1/\sqrt{L} \sum_k e^{i k x} g_k$ is different from zero only sites $x=0, 1, ..., m$ with $m$ the ``size'' of the scatterer.  No more assumptions on the model  will  invoked to prove our results, making our statements rather general.


We are interested in discussing the scattering.  Therefore, we need the the $S$-matrix, defined as,
\begin{equation}
( S_{y_1 ...y_n x_1...x_N} )_{\mu \nu} = \langle \lambda_\nu | \langle y_1, ..., y_N | \lim_{t_\mp  \to \mp \infty} U(t_-, t_+) | x_1, ..., x_N \rangle | \lambda_\nu \rangle
\end{equation}
Here, $|x_1, ...,x_N\rangle$ ( $|y_1, ...,y_N\rangle$)  are  elements of the position basis in the $N$-photon subspace,  $\\nu \rangle$  ($|\mu\rangle$)  states of the scatterer and $U (t_-, t_+) = e^{-i H t}$ is the evolution operator. What we show here is that in the light-matter Hamiltonian \eqref{H-full} it is possible to show that whenever $|x_i -x_j| \to \infty$ and $|y_i -y_j| \to \infty$ we have
\begin{equation}
\label{S0-main}
( S_{y_1 ...y_n x_1...x_N} )_{\mu \nu} \to
...
To find the above result we have shown some intermediate theorems.  We believe them important, even beyond the motivation in the $S$-matrix.  We enumerate the two main ones here.
