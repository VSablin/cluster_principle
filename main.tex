...
In waveguide QED the relevant model is the  quantised light-matter interaction $(\hbar=1)$, 
\begin{equation}
H = \sum_k \omega_k a_k^\dagger a_k + H_{sys} + \sum_k g_k (S^\dagger a_k + S a_k^\dagger).
\label{H-full}
\end{equation}
where $\omega_k$ are the photon frequencies and $[a_k, a_k^\prime]= \delta_{k, k^\prime}$ are bosonic operators.  We assume a single band photons:  $\omega_k \in [\omega_-, \omega_+]$  (multiband systems do not modify our conclusions). 
Besides,  $k = n \pi /L$ with $n=-L/2, ..., 0, ... L/2$, \emph{i.e.} with momentum spacing $\delta_k = \pi / L$.  The system-bosons coupling  is via the operator $S$ at strengths  $g_k$.   $H_{sys}$ is the system Hamiltonian that can be any.  
...
\end{equation}
the punctual scatterer is assumed to occupy a finite number of sites, \emph{i.e.} $g_x = 1/\sqrt{L} \sum_k e^{i k x} g_k$ is different from zero only sites $x=0, 1, ..., m$ with $m$ the ``size'' of the scatterer. 
 If necessary,  the continuum limit of the model can be done with the replacements $\sum_k \to \int  dk$ via $a_k \to \sqrt{\delta_k} a(k)$ ($[a (k), a^\dagger (k^\prime)]= \delta(k-k^\prime)$) and $g_k \to \sqrt{\delta_k} g(k)$. No more assumptions on the model  will  invoked to prove our results, making our statements rather general. It is worth to insist that \eqref{H-full} models all the experimental implementation enumerated in the introduction.
...
\end{equation}
Here, $|x_1, ...,x_N\rangle$ ( $|y_1, ...,y_N\rangle$)  are  position eigenstates in the $N$-photon subspace,  $|\nu \rangle$  ($|\mu\rangle$)  eigenstates of the scatterer and $U (t_-, t_+) = e^{-i H (t_+-t_-)}$ is the evolution operator.  
...
For simplicity,   above, we have considered chiral waveguides with only right ( or left) moving photons are considered.   The general case of non-chiral waveguides are easily done as we explain section \ref{sec:}.   Even with this simplification  the formula may appear obscure.  However, it  is rather simple in words.   Consider  two input  wavepackets (see figure \ref{fig:input} ).  Imagine that they are well separated,  eventually they can be arbitrarily separated.  Eq. \eqref{S0-main} says that, in this case,  the output will consists on the sum of single wavepacket events.  They are so separated that they do interact with the scatterer one after the other.  Since the scatterer, after the interaction with single photon wave packets, can be left in some state $\lambda_n$ we should sum over all these intermediate states.  Finally, the Heaviside is there because of causality (it orders the different single photon events).   If the system, after each scattering event, relax to its ground state  \eqref{S0-main} reduces to:
\begin{equation}
...
In this section we establish an important feature of the ground state of \eqref{H-full}.  In plain words, we show that  the expected values for local observables acting over a   region  away from the scatterer are arbitrarily well approximated by averages done within the free field vacuum (the ground state for $H= \sum_k \omega_k a_k^\dagger a_k$).  This statement needs of two theorems that we enunciate and demonstrate here.
...
\end{equation}
with $O$ an operator.   Noticing that
\begin{align}
...
\end{align}
where we have used that $H \geq 0$.  


We are going to create another state which is arbitrarily close to it by adding a small perturbationqq
\begin{equation}
...
As a consequence, we get what we want encapsulated in the following theorem.

\begin{theorem}
\label{eq:bound-x}
Let us define $\psi_{k_0}(x)$ as the wavepacket\ \eqref{eq:wavepacket} where $\phi_{k_0}(k)=\xi(k-k_0)$ is a test function, infinitely differentiable with a finite support $K$ centered around $k_0$. Let us assume that $|g_k/\omega_k|$ remains finite within that support. Then
\begin{equation}
\braket{\psi_{k_0}(x)^\dagger \psi_{k_0}(x)} \to 0,~|x|\to\infty.
\end{equation}
Once more this testifies that $\ket{\Omega}$ is a good vacuum, now far from the interaction region, because $\Vert{\psi_{k_0}(x)\ket{\Omega}}\Vert \to0$ as $x\to\pm\infty$. Moreover, if we can assume that $\braket{a_k^\dagger a_p}$ is an $n$-times differentiable function of $k$ and $p$, the bound will be improved
\begin{equation}
...
\end{equation}
We are now going to assume that $\phi_{k_0}(k)$ is a test function with compact support $K$ of size $|K|$ centered around $k_0$, and infinitely differentiable. We will also assume that within its support $|g_k/\omega_k|\leq C_\phi$ for some constant $C_\phi$. Then we can bound
\begin{equation}
\int |F(u)|\mathrm{d}u \leq |K|^2 C_\phi.
\end{equation}
Using the Riemann-Lebesgue theorem, we have then that
\begin{equation}
\left|\int e^{i u p} F(u)\mathrm{d}u\right| \leq \mathcal{O}\left(|x|^{-1}\right)
\end{equation}
at long distances. 
{\color{red} If we can ensure that the expectation values of the $\braket{a^\dagger_k a_p}$ operators is $n$-times differentiable, then we have that $F(u)$ is also $n$-times differentiable and we can reuse the ideas from Lemma\ \ref{lemma:bounds} to achieve a better decay.}
\end{proof}
...
Let us move to dynamics and discuss what causal relations mean in a dispersive media.  For that, we will work with localized wavepackets $\psi_{k_0}(x,t)$ as defined in \eqref{wp}.  Actual calculations are done with gaussian wavepackets:
\begin{equation}
\phi_{k_0}(k) = \frac{1}{\sqrt[4]{\pi}\sqrt{\sigma}}
\exp\left[-(k-k_0)^2/2\sigma^2\right].
\label{eq:gaussian}
\end{equation}
wich means that \eqref{wp} can explicitely written in real space as,
\begin{equation}
---
\end{equation}
\emph{i.e.} a state localised arround ...


\begin{lemma}
\label{lemma:cones}
Let the dispersion relation $\omega_k$ have a non-negative, but upper bounded group velocity
\begin{equation}
0 \leq v_k = \partial_k \omega_k \leq c.
\end{equation}
Then, the function $f(k) = k x - \omega_k t$ only has a stationary points if the distance to the light-cone is non-negative. In other words
\begin{equation}
d_c(x,t) = \sgn(x)(x - c t) > 0 \Rightarrow \vert f'(k) \vert > 0,\: \forall k.\label{eq:distance}
\end{equation}
\end{lemma}

\begin{proof}
Solving the equation $f'(k)=x - \partial_k \omega_k t = 0$ leads to the condition $\frac{x}{t} = \partial_k \omega_k$. Since we are assuming that $\partial_k \omega_k \geq 0$ it follows that $0 \leq \frac{x}{t} \leq c,$ which implies that $x$ and $t$ must have the same sign. If $x > 0$, it immediately follows that $x \leq c t$ and $d \leq 0$. If $x < 0$, the sign of the inequality is inverted, $x \geq c t$, or $x - ct \geq 0$ implying once more $d \leq 0$.
\end{proof}
...
\label{eq:free-com}
\end{align}
If $d_c= d_c(x-y,t-t')  = d_c(y-x,t'-t) = \sgn(x-y)(x-y -c(t-t')) > 0$, using Lemma\ \ref{lemma:cones} we know that the exponent has no stationary point. We use this fact and the notation  $\tilde\omega_k = \omega_k -c k,$ and we rewrite
\begin{eqnarray*}
...
&=& \int  e^{ik(x-y-c(t-t'))-i\tilde\omega_k(t-t')}  \phi_{k_0}(k)\phi_{p_0}(k)^*\mathrm{d}k\\
&=&\int e^{ik\, \sgn(x-y) d_c(x-y, t-t')-i\tilde\omega_k(t-t')} \phi_{k_0}(k)\phi_{p_0}(k)^*\mathrm{d}k
\end{eqnarray*}
...
\right]\,S(\tau)\mathrm{d}\tau'.
\end{align}
The first part corresponds to free evolution, while the second part is an error term $\varepsilon(t)$ which can be bounded. We will assume without loss of generality $\Vert{S}\Vert=1$ and $|t_1|>|t_0|$. We have to choose the integration limits $t$ and $t_s$ so that $\sgn(\tau')=\sgn(x)$. If $x>0$ then $t_1>t_0>0$ and $(t,t_s)=(t_1,t_0)$ is a good choice. If $x<0$ then $0>t_0>t_1$ and again $(t,t_s)=(t_1,t_0)$ is also a valid choice ($\tau'<0$). This means we can introduce $\tau''=\sgn(x)\tau'\geq 0$ and bound
\begin{align}
|\varepsilon(t_1)|
&\leq \int_{0}^{|t_1-t_0|} \left|\int e^{i\sgn(x)kd_c(|x|,\tau'')}q(k)\mathrm{d}k\right|\mathrm{d}\tau''
