\documentclass[aps,pra,preprint,amsmath,amssymb]{revtex4-1}
...
Causality is  expected to hold in every circumstance.    In words of Weinberg, two uncorrelated experiments must provide uncorrelated results \cite[Chap. 4]{weinberg1995}.     In Quantum Field Theory, causality imposes that all spacelike separated operators conmute.   In non-relativistic quantum mechanics, however, we should abandon such an stringent condition which, obviously, does not implies any causality violation.  In local Hamiltonians\footnote{interactions decay sufficiently fast with the distance} there is a  maximum but finite group velocity in which the information can travel.   The only care is that operators do not conmute but their conmutator  (its norm) is arbitrarily small in distances large enough compared to this maximum velocity \cite{Lieb1972}.  The latter, and its variances, is the known Lieb-Robinson bounds that state what causality means without the Lorentz invariance. 
{\color{red} decir algo de non signaling?}

Lieb and Robinson, with their paper,  brought about that several results, rigorous in QFT, were verified in many body quantum mechanics such as the exponential decay of correlations (at equilibrium)  in gapped models or  the area law \cite{Nachtergaele2006, Hastings2007}.   Another key object  to look is the $S$-matrix.  Scattering experiments  are the way of probing system.  Thus, the postulates in the theory are reflected in the properties  of the $S$-matrix.   In a  more strong way, experiments dictate the acceptable form of the latter.  At the most fundamental level, causality imposes that the output of uncorrelated inputs is a sum of each of them in a convenient way.   Under some conditions (to discuss below) it reduced to a  cluster decomposition for the $S$-matrix \cite{Wichmann1963}.  Well behaved interactions in QFT  can be argued by imposing such a decomposition for the scattering \cite{weinberg1995}.  
...
In this work, we are going to considered the situation occurring in waveguide QED.  There, travelling photons interact with local quantum systems.   The photonic medium, in general, will be dispersive
{\color{red} dispersive = absorving?} and the coupling with the quantum systems may be non-perturbative.  In this sense, the problem we cast here is a many body one.  Besides, typical assumptions as Lorentz or Translational invariance can not be done here.  Although theoretically motivated, it is worth recallying that the scenario discussed here is experimentally relevant nowadays.   On the other hand,   waveguide QED experiments can test such causality restrictions for the $S$-matrix.  
We argue the  form for $N$-photon $S$-matrix acceptable under causality conditions, also fully developed here.   We emphasize that our results do not rely in typical approximations as Markovian and/or weak scatterer-photonic medium coupling.  Our results are also independent of the scatterer.    Our results are accompanied with several theorems and tested in two examples where, either analytically (the case of  a non-dispersive medium interacting with a lambda atom) or numerically (scattering in the ultrastrong limit) has been recently discussed in the literature.   Both models share the property of  presenting Raman scattering which introduces several subtitles. 
