...
It would be desirable to proof such a decomposition in non-relativistic many body systems.  It should not be  surprising that such a thing occurs, as we have just reviewed, a maximuum velocity for transfer quantum information guarantees causality.  Checking this expected result is the purpose of this paper.
...
In this section, we establish an important property of the ground state, $|\Omega\rangle$,  of \eqref{H-full}.  We show that  the expected values for local observables acting over a   region  away from the scatterer are arbitrarily well approximated by averages done within the free field vacuum (the ground state for $H= \int \; dk \, \omega_k \, a_k^\dagger a_k$).  This statement needs of two theorems that we enunciate and demonstrate here.
...
\begin{align}
\braket{a_k^{\dagger\,n} (-n\omega_n a_k^{n} -n g_k S a_k^{n-1}}\geq 0,
\end{align}
or equivalently
\begin{equation}
0 \leq \braket{a_k^{\dagger\,n}a_k^n} \leq -\frac{g_k}{\omega_k}\braket{ S a_k^{\dagger\,n}  a^{n-1}}.
\end{equation}
If we introduce $C_n := |\braket{a_k^{\dagger\,n}a_k^n}|$, with $C_0=1$, Cauchy-Schwatz translates this into the upper bound
\begin{equation}
0\leq C_n \leq \frac{|g_k|}{\omega_k} \sqrt{ \langle S^\dagger S \rangle C_n C_{n-1}}.
\end{equation}
In other words, $C_n \leq C_{n-1}|g_k/\omega_k|^2\langle S^\dagger S \rangle $.
\end{proof}


This result allows to show what we announced at the beginning of the section.
