...
\begin{lemma}
\label{lemma:cones}
Let the dispersion relation $\omega_k$ (its absolute value) have an  upper bounded group velocity
\begin{equation}
\vert  v_k = \partial_k \omega_k  \vert \leq c.
\end{equation}
...
\begin{equation}
d_c(x,t) = ( | x | - c | t | ) > 0 \Rightarrow | f^\prime (k) | > 0,\: \forall k.\label{eq:distance}
\end{equation}
\end{lemma}

\begin{proof}
Solving the equation $f'(k)=x - \partial_k \omega_k t = 0$ leads to the condition $\frac{x}{t} = \partial_k \omega_k$.  To fullfil this, If $\partial_k \omega_k \geq 0$:  $x/t >0$ and  $|x|/|t|=\partial_k \omega_k$.  It follows that $0 \leq |x|/|t| \leq c$ wich implies \eqref{eq:distance}.  On the other hand,  If $\partial_k \omega_k \leq 0$:  $x/t <0$ and  $|x|/|t|=-\partial_k \omega_k$.  Again, the same inequality follows $0 \leq |x|/|t| \leq c$,  showing   \eqref{eq:distance} in all cases.
\end{proof}
...
Then, the function $f(k) = k x - \omega_k t$ only has a stationary points if the distance to the light-cone is non-negative. In other words
\begin{equation}
d_c(x,t) = {\rm sign}(x)(x - c t) > 0 \Rightarrow | f^\prime (k) | > 0,\: \forall k.\label{eq:distance}
\end{equation}
\end{lemma}
