...
&=& \int  e^{ik(x-y-c(t-t'))-i\tilde\omega_k(t-t')}  \phi_{k_0}(k)\phi_{p_0}(k)^*dk\\
&=&\int e^{ik\, \rm{sign}(x-y) d_c(x-y, t-t')-i\tilde\omega_k(t-t')} \phi_{k_0}(k)\phi_{p_0}(k)^*dk
\end{eqnarray*}
...
For the sake of simplicity, we consider just two photons...

{\color{red}Aquí falta el desarrollo QUE AHORA MISMO NO SÉ CÓMO HACER.}
